...
**Dictionary cost (P3b, RECALIBRATED 2026-06-18):** L(D) = sum over entries of `decl(symbol)`, where `decl(symbol) = elias_gamma_len(arity + 1)` -- the bit length of the Elias gamma code of (arity + 1). Constants are arity-0 leaves (1 bit); arity-1 and arity-2 functors cost 3 bits; arity-3 and arity-4 cost 5 bits. This is RENAME-INVARIANT: a symbol's declaration cost depends only on its arity, never on how its name is spelled. It is the one-time declaration charge: a proposal introducing a NEW symbol pays its arity-code into the dictionary; removing a symbol's LAST occurrence credits it back.

> **Why this replaced the original `8 * len(name) + 8` charge.** The first cut priced a symbol by the ASCII length of its name. The decision-diff (Section 5) showed this was rename-variant and pathological: a verbose auto-generated name like `exception_likes_chocolate_person` cost 272 bits, which swamped the structural savings, so under bits mode almost NO abstraction paid (18 of 24 symbolic compressions that clause-count accepts were rejected). The defect is conceptual: an MDL code for a KB should count how many distinct symbols and clauses exist, not how their (arbitrary, generator-assigned) labels are spelled. The arity-gamma charge restores rename-invariance, is parameter-free (no tunable constant), and lands the symbolic acceptance rate at a principled bar (16 of 24 accepted; the 8 rejections are abstractions that genuinely add net symbol occurrences). The per-occurrence `log2(|F|)` / `log2(|C|)` payloads already make the COUNT of distinct symbols enter the cost, so the dictionary charge only needs to declare each symbol's arity.
